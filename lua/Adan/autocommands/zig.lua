...
vim.api.nvim_create_autocmd('FileType', {
    ...
    callback = function(args)
        ...
        vim.keymap.set('n', '<leader>lf', ':!zig fmt %<CR>',
            { buffer = bufnr, desc = 'Format file' })

        vim.keymap.set('n', '<leader>lbp', function() zig_make('zig build') end,
            { buffer = bufnr, desc = 'Build project' })

        vim.keymap.set('n', '<leader>lbr', function() zig_make('zig build run') end,
            { buffer = bufnr, desc = 'Build and run' })

        vim.keymap.set('n', '<leader>lbt', function() zig_make('zig build test') end,
            { buffer = bufnr, desc = 'Build and test' })
